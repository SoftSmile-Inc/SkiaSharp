...
- **zlib's inclusion is largely redundant.** The vendored zlib fork already renames essentially everything (151 `#define`s in `chromeconf.h`, including internal helpers) to `Cr_z_*`. A host app's own *plain* zlib cannot collide with `Cr_z_*` names regardless of this mechanism; the extra `sksharp_Cr_z_*` layer only helps in the narrow case where the host also bundles a Chromium-forked zlib. Kept for completeness/uniformity across the four libraries, not because a real gap was found. (Confirmed empirically against a real Unity WebGL build: Unity bundles plain `zlib`/`inflate`, not a Chromium fork — zero `Cr_z_*` names found — so this layer is provably inert for that project specifically, not just in theory.)
