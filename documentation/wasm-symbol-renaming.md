...
An opt-in build-time mechanism that avoids duplicate-symbol linker errors when the wasm `libSkiaSharp.a` static library is linked into a host application that statically links its own copies of freetype2, libjpeg-turbo, zlib, or libpng.

## TL;DR

- **Problem:** `libSkiaSharp.a` (wasm) and a host app's own freetype2/libjpeg-turbo/zlib/libpng export the same global C symbols. Statically linking both into one binary fails (or silently picks the wrong copy).
...
- Relevant files: `native/wasm/build.cake`, `native/wasm/libSkiaSharp/wasm_symbol_renames.h` (generated), `scripts/Docker/wasm/build-local.sh`, `scripts/Docker/wasm/generate-symbol-renames-local.sh`.
...
- **Git tracking of the generated header is still undecided.** `native/wasm/libSkiaSharp/wasm_symbol_renames.h` remains a tracked file. Since it is now rewritten by every build that enables renaming, local builds will show it as modified even when its content is materially unchanged. Whether to keep committing it (for human review of what changes after a DEPS bump) or move it to an untracked build-output path has not been decided.
