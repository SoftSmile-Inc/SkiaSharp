...
description: >
  ...
  including Component Governance (CG) alerts and legacy TSA Azure Boards work items for
...
4. **`findings`** — Array of finding objects sorted by priority then severity. **ONE object per audited product dependency in `versionVerification`** (e.g., one "skia" finding containing ALL Skia CVEs regardless of status). Each has `dependency`, `status`, `cves[]`, `nonChromeCves[]`, `action`, `notes`. The `status` reflects the WORST-case status among the CVEs. Do not duplicate Component Governance, TSA, or release-schedule actions as synthetic findings; they have dedicated sections and belong in `nextSteps`.
